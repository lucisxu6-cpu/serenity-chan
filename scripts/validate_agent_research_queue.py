...
def validate_agent_research_queue(payload: Mapping[str, Any]) -> list[str]:
    ...
    missing_symbols: list[Any] = _as_list(payload.get("missing_ai_result_symbols"), "missing_ai_result_symbols", errors)
    if not missing_symbols:
        errors.append("missing_ai_result_symbols must contain at least one symbol")
    work_items: list[Any] = _as_list(payload.get("work_items"), "work_items", errors)
    if not work_items:
    ...
    for index, item in enumerate(work_items):
        ...
        if item.get("required_action") != "produce_validated_ai_overlay_or_outcome":
        ...
        allowed_results: list[Any] = _as_list(item.get("allowed_results"), f"work_items[{index}].allowed_results", errors)
        ...
        for key in ["manifest_path", "review_packet", "committee_packet", "overlay_prompt", "overlay_output_path", "outcome_output_path"]:
            if not _non_empty_string(item.get(key)):
                errors.append(f"work_items[{index}].{key} must be non-empty")
        if len(_as_list(item.get("validation_commands"), f"work_items[{index}].validation_commands", errors)) < 2:
            errors.append(f"work_items[{index}].validation_commands must contain overlay and outcome validators")
        if len(_as_list(item.get("guardrails"), f"work_items[{index}].guardrails", errors)) < 3:
