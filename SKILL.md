...
`fetch` 能自动抓取可用的 L2 行情/复权历史、美股 SEC L0 财报/filing、A 股 Eastmoney/Tencent L2 行情/前复权 K 线、A 股 CNINFO L0 公告元数据、A 股 CNINFO L0 官方报告 PDF 核心财务行抽取、A 股金融行业专门 profile，以及 A 股 Eastmoney F10 L3 回退财务预检，并生成带 `raw_hash` / `pdf_hash` 的本地数据包。美股 CIK 解析先使用稳定 bootstrap，再尝试 SEC ticker 目录；财务先取 SEC companyfacts，必要时继续取 SEC companyconcepts。若网络、证书、SEC 身份、源限流或市场不支持导致 fetch 失败，必须把相应字段标记为 `FAILED` / `PENDING` 并写入 `attempt_ledger`；若 scoped fetch 未请求某数据，标记 `NOT_REQUESTED` 并写入 `data_gaps`。这些状态都不能支持高评级；不得改用猜测值。
...
- `Tencent_Quote_Kline_L2`：A 股 SH/SZ/BJ 当前行情和日线历史补充源；不能把未确认日线直接当作前复权数据。
- `Yahoo_Chart_L2` / `Yahoo_Chart_Query2_L2`：US / HK 行情和历史，以及 A 股辅助兜底行情源。
- `SEC_Companyfacts_L0`：美股 SEC companyfacts 和 submissions，属于 L0 官方源。
- `SEC_CompanyConcepts_L0`：美股 SEC companyconcepts 细粒度财务事实，属于 L0 官方源，用于 companyfacts 大包不可用时继续取数。
...
- `Eastmoney_F10_Financials_L3`：A 股三表 L3 结构化回退预检，输出收入、归母净利、经营现金流、资产、负债、权益等核心字段；仅 F10 可用时必须保留财报验证债务，不能放开 S/A 研究评级。
...
3. AI 再补入产业链层级、公司卡点、客户/订单、分部收入和估值反推等需要领域判断的字段。
4. 同一正式评级上限下，仍必须输出候选优先级差异、研究债务差异、资本动作差异和技术动作差异。
5. 若当前价和股本可用，`growth_hypothesis_matrix` 先输出预检级市值、PE/PS 和 H 档；缺少估值输入时保持 `UNKNOWN`，并生成估值补证任务。
...
|---|---|---|---|---|
| A 股 | 688019.SH / 300750.SZ / 920593.BJ | 巨潮、上交所、深交所、北交所 | CNINFO 官方报告 PDF 行级财务抽取 → 银行等金融行业 profile → Wind/Choice/CSMAR/Tushare → Eastmoney/Tencent/AKShare/BaoStock 行情辅助 → Eastmoney F10 L3 回退预检 | 复权口径、F10 错误、互动平台弱证据、金融企业口径错配 |
| 美股 | AAPL / NVDA / MU | SEC EDGAR、公司 IR | Polygon/FactSet/Bloomberg/Tiingo/Yahoo/Stooq；SEC XBRL 财报 | split、ADR、非 GAAP、SBC、ATM/S-3 |
| 港股 | 0700.HK / 9988.HK | HKEXnews、公司公告 | HKEXnews 年报/中报 PDF → Wind/Choice/港交所/财经数据商 | 港币、流动性、配售、A/H 差异 |
...
- 行动状态：关键研究债务未清时保持 `DATA_GATED` / `WAIT_FOR_BUY_POINT` / `LEAD_TRACKING`。
...
8. capital_actions。
9. research_debt。
10. candidate_priority_ranking 与 final_decision。
