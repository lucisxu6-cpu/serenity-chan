...
7. 若当前价和股本可用，`growth_hypothesis_matrix` 先输出预检级市值、PE/PS 和 H 档；缺少估值输入时保持 `UNKNOWN`，并生成估值补证任务。
...
候选对比必须包含 `comparison_output_contract` 的十个矩阵：
...
6. growth_hypothesis_matrix。
7. technical_timing_matrix。
8. capital_actions。
9. research_debt。
10. candidate_priority_ranking 与 final_decision；ranking 行必须包含 `research_priority_score`、`action_priority_score` 和带 `primary_gate_class` 的 `action_gate`。
