...
- `laplace_strategy_input`：当用户需要趋势判断、可操作方向、组合/仓位、30/90/180 天触发器或复盘时，将候选对比报告转换为 companion Laplace 策略输入。
...
3. 非内置主题由 AI 先按 `assets/theme_candidate_universe.schema.json` 写出价值链层级、候选宇宙、热门降级方向和扩展任务，再运行 `validate_theme_candidate_universe.py`；不得用占位候选替代真实候选。
4. 先排产业链层级，再排公司。
5. 至少覆盖 3 个价值链层级；深度扫描尽量建立 20+ 候选公司。
6. 每个 top candidate 必须说明“卡住什么环节”。
7. 至少给出一个热门但降级的方向，并说明为什么。
8. 当前结论必须用最新数据源；没有数据时标记为 initial pass。
9. 如果直接列股票而不输出产业链地图、瓶颈层级排序和候选池，视为不合格输出。
...
5. 顶层流程在最终报告生成后自动写出 `laplace_strategy_input.json`；用户询问趋势、推荐、组合、仓位、行动触发器或后续怎么做时，继续进入 companion Laplace 策略层。
6. 需要手工拆分时，才使用低层命令：`data_router.py fetch`、`build_comparison_report.py`、`build_ai_overlay_prompt.py`、`build_ai_review_packet.py`、`build_ai_committee_packet.py`、`validate_and_merge_ai_overlay.py`、`render_research_report.py`、`build_laplace_strategy_input.py`。
7. `validate_and_merge_ai_overlay.py` 会强制要求每个候选都有一个正式 AI 结果，并会把 overlay 的 `source_ref` 解析到对应 manifest 的真实 source artifact 或 AI review packet；无法解析的引用、与证据不一致的数字 claim 会阻断合并。
8. 用 `python scripts/render_research_report.py --comparison-report comparison_report.json --mode candidate_comparison` 输出候选对比交付稿；需要逐候选研究工作台时使用 `--mode full_research`。
9. 同一正式评级上限下，仍必须输出候选优先级差异、研究债务差异、资本动作量化差异、技术动作差异和候选池语义一致性；只有同层候选可进入正式 clear top candidate，同主题不同层、跨主题或无关诊断只能作为研究优先级或诊断集合。
10. 若当前价和股本可用，先在 `valuation_input_matrix` 暴露价格、股本、市值、货币、日期、来源、口径和 `valuation_stage`，再由 `growth_hypothesis_matrix` 引用该估值输入并输出预检级 PE/PS 和 H 档；缺少估值输入时保持 `UNKNOWN`，并生成估值补证任务。
11. 财报行若以 million、thousand、万元等单位披露，必须先归一到绝对金额，再计算 PE/PS 和市场隐含增长。
12. 若 `data_consumption_audit` 出现 `MISMATCH`，不得输出正式 top candidate；先修复数据消费链路。
...
1. 先完成 Serenity 数据和 AI 研究闭环，得到 validated comparison report；若用户只要宏观趋势或行业判断，也必须明确 observed / inferred / judgment 边界。
2. 运行 `python scripts/build_laplace_strategy_input.py <comparison_report.json> --out <laplace_strategy_input.json>`。
3. 运行 `python scripts/validate_laplace_strategy_input.py <laplace_strategy_input.json>`。
4. 读取 `references/16_laplace_strategy_bridge.md`。
5. 读取 `companion-skills/laplace-forecast/SKILL.md`；行业、市场或组合问题同时读取 `first-order-lenses.md`，证据不足或需要代理指标时读取 `evidence-loop.md`，需要复盘时读取 `ledger-schema.md`。
6. 输出中文策略结果，显式拆分 `Observed`、`Inferred`、`Judgment`，并包含 Forecast、Decision、Decision model、Dominant variables、Scenarios、Triggers、Invalidation、Next evidence 和 Action plan。
7. 重要主题、候选池或中期策略判断必须生成可写入 forecast ledger 的结构化 claim；无法定义 claim 时说明证据天花板和下一观察点。
8. Serenity 的硬门控不能被策略层覆盖；研究债务只能转成下一证据、触发器或失效条件。
...
20. 用户要求趋势、推荐、组合或行动方案时，必须基于 `laplace_strategy_input` 进入 strategy forecast 输出。
...
```bash
python scripts/build_laplace_strategy_input.py <comparison_report.json> --out <laplace_strategy_input.json>
