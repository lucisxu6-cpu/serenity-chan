...
6. AI 研究结果必须压缩成 `assets/ai_research_overlay.schema.json` 允许字段，并运行 `python scripts/validate_ai_overlay.py <overlay.json>`；委员会研究记录只能映射进 `posterior_basis`、`contrary_evidence`、`required_next_evidence`、`research_questions` 等 overlay 字段。
...
9. capital_actions。
10. data_consumption_audit。
11. research_debt。
12. candidate_priority_ranking；ranking 行必须包含 `research_priority_score`、`action_priority_score` 和带 `primary_gate_class` 的 `action_gate`。
13. final_decision；必须包含 `decision_mode`、`score_gap_to_runner_up`、`ranking_validity` 和候选池数量提示。

`ranking_validity=INVALID` 时，只能输出工程诊断和补数/修复任务；`ranking_validity=PARTIAL` 时，可以输出研究优先级，但不能输出 `clear_top_candidate`。
