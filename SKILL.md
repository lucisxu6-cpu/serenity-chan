...
9. 用户可读分析结果默认使用中文；机器字段可保留英文枚举，但必须用中文解释状态、限制和下一步。
...
4. 用 `python scripts/build_ai_review_packet.py <manifest> --out <packet.json>` 生成 AI 审阅包。
5. 用 `python scripts/build_ai_committee_packet.py <manifest> --out <committee.json>` 生成多角色 AI 研究委员会包；`consensus`、`dissent`、`upgrade_conditions`、`downgrade_conditions` 属于委员会研究记录。
6. AI 研究结果必须压缩成 `assets/ai_research_overlay.schema.json` 允许字段，并运行 `python scripts/validate_ai_overlay.py <overlay.json>`；委员会研究记录只能映射进 `posterior_basis`、`contrary_evidence`、`required_next_evidence`、`research_questions` 等 overlay 字段。已有委员会输出可先用 `python scripts/committee_to_overlay.py <committee_output.json> --out <overlay.json>` 收敛，但脚本不得凭空生成研究判断。
7. 用 `python scripts/merge_ai_research_overlay.py <manifest...> --overlay SYMBOL=<overlay.json> --format json > comparison_report.json` 生成带 AI 覆盖层的候选对比 JSON，并再次运行 `python scripts/validate_comparison_report.py comparison_report.json`。
...
9. 同一正式评级上限下，仍必须输出候选优先级差异、研究债务差异、资本动作差异和技术动作差异。
...
7. currency_normalization_matrix。
8. growth_hypothesis_matrix。
9. technical_timing_matrix。
10. capital_actions。
11. data_consumption_audit。
12. readiness_matrix。
13. research_debt。
14. candidate_priority_ranking；ranking 行必须包含 `research_priority_score`、`action_priority_score`、`decision_grade` 和带 `primary_gate_class` 的 `action_gate`。
15. final_decision；必须包含 `decision_mode`、`score_gap_to_runner_up`、`ranking_validity` 和候选池数量提示。
