...
这套设计让 AI 保留充分的研究空间，同时把结论锁定在可复核的证据链上。证据充分时，AI 会把深研档案投影成正式研究结论；证据不足或与确定性数据冲突时，AI 会留下失败原因、冲突字段、下一证据和继续研究路径。
...
serenity-chan 会区分两类输出：

| 输出层 | 作用 |
|---|---|
| 研究简报 | 展示当前已知事实、候选排序、主要门控、待补证据和下一步研究动作 |
| 正式报告 | 在 AI 深研、证据投影和交付校验完成后，输出可复核的候选比较与策略输入 |
...
This gives AI room to reason deeply while keeping conclusions tied to reviewable evidence. When evidence is strong enough, AI projects the research record into a formal conclusion; when evidence is insufficient or conflicts with deterministic data, AI records the reason, conflicting fields, next evidence, and continuation path.
...
serenity-chan separates two output layers:

| Output Layer | Purpose |
|---|---|
| Research Brief | Shows known facts, candidate ranking, main gates, missing evidence, and next research actions |
| Formal Report | Delivered after AI research, evidence projection, and delivery validation are complete |
