...
A 股财报优先使用 `CNINFO_FinancialReports_L0` 官方报告 PDF 行级抽取。普通经营企业需说明收入、利润、经营现金流、资产、负债、权益的抽取状态；银行、证券、保险等金融企业还需说明专门 profile 是否覆盖行业核心指标。若财报只来自 `Eastmoney_F10_Financials_L3`，只能作为 L3 结构化预检，必须保留财报验证债务，最终研究评级上限为 `B`。
...
| Action Gate Class | DATA_ACQUISITION / RESEARCH_VALIDATION / ACTION_TIMING / NONE | |
...
优先研究：____。
决策模式：clear_top_candidate / tentative_top_candidate / candidate_cluster。
与第二名分差：____。
候选池数量提示：____。
当前动作：____。
...
## 4. 估值输入矩阵
| 公司 | 状态 | 当前价 | 总股本 | 总市值 | 货币 | 来源 | 股本口径 | 市值口径 | 验证需求 |
|---|---|---:|---:|---:|---|---|---|---|---|
...
## 9. 行动门控
| 公司 | Primary Gate | Secondary Gates | Blocking Datasets | Reasons |
|---|---|---|---|---|
