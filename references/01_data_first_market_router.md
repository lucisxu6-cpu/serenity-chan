...
AI research results are the formal bridge from deterministic data to domain judgment. Build a prompt package with `scripts/build_ai_overlay_prompt.py`, build supporting packets with `scripts/build_ai_review_packet.py` and `scripts/build_ai_committee_packet.py`, then produce one validated dossier plus one validated projected result per candidate. Full research uses `assets/ai_research_dossier.schema.json` and `scripts/validate_ai_research_dossier.py`. Evidence-backed projection uses `assets/ai_research_overlay.schema.json` and `scripts/validate_ai_overlay.py`. Insufficient evidence, deterministic-data conflict, and quick audit use `assets/ai_review_outcome.schema.json` and `scripts/validate_ai_review_outcome.py`. Merge dossier and projected result types through `scripts/validate_and_merge_ai_overlay.py`, which also validates the final comparison report. The dossier preserves source reading, research path, hypotheses, evidence tests, observed/inferred/judgment, claim graph, causal chain, scenarios, triggers, and action conditions. The overlay can affect layer score, company fit, evidence-supported growth, contrary evidence, research questions, and bounded AI deltas only after validation. Market-implied growth and growth gap are generated from valuation inputs, PE/PS, and evidence-supported growth in the comparison report.
